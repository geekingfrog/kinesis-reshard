...
data Operation
    = Split Shard !Integer -- shard, exclusiveStartKey
    | Merge (Shard, Shard)
    | Noop Shard
    deriving (Show, Eq)


reshard :: Int -> Stream -> [Operation]
reshard targetNumShard stream =
  let
    finalStream = makeStream targetNumShard (streamStart stream) (streamEnd stream)
    tmpStream = intermediateStream targetNumShard stream
    splitParts = assocSplits (streamShards stream) (streamShards tmpStream)
    splitOps = concatMap (uncurry splitShards) splitParts
    mergeParts = assocMerge (streamShards tmpStream) (streamShards finalStream)
    mergeOps = concatMap (uncurry (flip mergeShards)) mergeParts
  in
    splitOps ++ mergeOps


makeStream :: Int -> Integer -> Integer -> Stream
makeStream shardNumber start end = Stream
    { streamStart = start
    , streamEnd = end
    , streamShards = shards
    }
...
intermediateStream :: Int -> Stream -> Stream
intermediateStream finalShardNumber startingStream =
  let
    finalStream = makeStream finalShardNumber (streamStart startingStream) (streamEnd startingStream)
    intermediateShards = mergeStream (streamShards startingStream) (streamShards finalStream)
  in
    startingStream {streamShards = intermediateShards}
...
assocSplits = assoc
-- assocSplits a b = filter (not . null) $ assoc' a b
--   where
--     assoc' [] _ = []
...
-- assocMerge _ [] = []
-- assocMerge shardsToMerge (x:xs) =
--   let
--     (toMerge, rest) = List.span (\sh -> shardStart sh < shardEnd x) shardsToMerge
--   in
--     (x, toMerge) : assocMerge rest xs
